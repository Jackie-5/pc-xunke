...
module.exports = {
  zixunCategory: async (ctx) => {
    ...
    if (!result) {
      ...
      try {
        ...
        debugData('consulCate', data);

      } catch (e) {
        ctx.throw(500, `接口返回错误 queryAllDataDictToCForShow ${e}`);
      }
    }
  ...
  getMainList: async (ctx) => {
    const { envLinks, basisCurrentDomain, currentCity } = ctx.state;
    try {
      const getData = await ctx.state.axios(`${envLinks.toCHost}/baike/queryConsulArticleOfLabels`, {
        cityCode: currentCity.id,
        domain: basisCurrentDomain.domain,
      }, {
        method: 'post'
      ...
      ctx.throw(500, `接口返回错误 /baike/queryConsulArticleOfLabels ${e}`);
    }
  },

  newZixunList: async (ctx, detailParentCode) => {
    ...
    try {
      const data = await ctx.state.axios(`${envLinks.toCHost}/baike/queryConsulArticle`, axisoParams, {
        method: 'post'
      });

      debugData('zixunLis1t', data);
  ...
  detailZixun: async (ctx) => {
    const { envLinks, debugData } = ctx.state;
    try {
      const code = ctx.params.code && ctx.params.code.includes('.html') && ctx.params.code.split('.html')[0];
      
      const data = await ctx.state.axios(`${envLinks.toCHost}/baike/getConsulArticleDetailByCode`, {
        id: code,
      });
  
      debugData('detailZixun', data);

      return data.data ? data.data : {};
    } catch (e) {
      ctx.logger.warn(`接口返回错误 /baike/get-article-detail ${e}`);
      ctx.throw(404);
    }
  },
};
